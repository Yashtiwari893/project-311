...
export default function ProfileManagerPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      setUserId(user.id)
      // First AI message
      const first = FLOW_STEPS[0].message(lang, {})
  ...
  const generateContent = async (profileData: ProfileData) => {
    try {
  ...
  const toneLabels: Record<string, string> = { '1': 'Professional & formal', '2': 'Friendly & approachable', '3': 'Bold & confident', '4': 'Technical & precise', '5': 'Creative & storytelling' }
        ...
        {generated && (
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: 20, animation: 'slideUp .4s ease' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ background: '#f8fafc', borderRadius: 8, padding: 12, fontSize: 13, color: '#0f172a', lineHeight: 1.6, border: '1px solid #e2e8f0' }}>
                {generated.headline}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ background: '#f8fafc', borderRadius: 8, padding: 12, fontSize: 12, color: '#0f172a', lineHeight: 1.7, border: '1px solid #e2e8f0', whiteSpace: 'pre-wrap', maxHeight: 180, overflow: 'auto' }}>
                {generated.about}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {['post1','post2','post3','post4','post5'].map((key, i) => generated[key] ? (
                  <div key={key} style={{ background: '#fef9c3', borderRadius: 8, padding: 12, fontSize: 12, color: '#0f172a', lineHeight: 1.6, border: '1px solid #fde68a' }}>
        ...
        <div style={{ background: '#fff', borderTop: '1px solid #e2e8f0', padding: '16px 24px', flexShrink: 0 }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end', background: '#f8fafc', borderRadius: 14, padding: '10px 12px', border: '1.5px solid #e2e8f0' }}>
            <textarea
              ...
              placeholder={typeof currentStepDef?.placeholder === 'function'
            ...
            />
            <button onClick={handleSend} disabled={sending || !input.trim()} style={{ width: 36, height: 36, borderRadius: 9, background: input.trim() ? '#4f72ff' : '#e2e8f0', border: 'none', color: '#fff', cursor: input.trim() ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>
              ↑
            </button>
          </div>
          <p style={{ fontSize: 11, color: '#94a3b8', textAlign: 'center', marginTop: 6 }}>
            {lang === 'hi' ? 'Enter से भेजें · Shift+Enter नई line' : 'Enter to send · Shift+Enter for new line'}
          </p>
        </div>
      )}
